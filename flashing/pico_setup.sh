...
if [[ "$SKIP_VSCODE" == 1 ]]; then
    ...
    if [ -f vscode.deb ]; then
        ...
        # Get extensions
        code --install-extension marus25.cortex-debug --user-data-dir="~/.vscode-root"
        code --install-extension ms-vscode.cmake-tools --user-data-dir="~/.vscode-root"
        code --install-extension ms-vscode.cpptools --user-data-dir="~/.vscode-root"
    fi
fi
